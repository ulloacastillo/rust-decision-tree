...
impl DecisionTreeClassifier {
    ...
    pub fn build_tree(&mut self, X: &Matrix, Y: &Vec<i32>, curr_depth: usize) -> Option<Box<Node>> {
        ...
        let depth: f32 = std::f32::INFINITY;

        if self.max_depth > 0 {
            let depth: f32 = self.max_depth as f32;
        }
    ...
    pub fn gini_index(&mut self, Y: &Vec<i32>) -> f32 {
        ...
        let gini_map: f32 = class_labels
            .iter()
            .map(|x| {
                let p_cls: f32 = ((utils::count_vals(&Y, &x) as f32) / (*y_len) as f32) as f32;
                p_cls * p_cls
            })
            .sum();

        // let gini_sum: f32 = class_labels.iter().fold(0.0, |mut acc, x| {
        //     let p_cls: f32 = ((utils::count_vals(&Y, &x) as f32) / (*y_len) as f32) as f32;
        ...
        for cls in class_labels {
            let p_cls: f32 =
                ((utils::count_vals(&Y, &cls) as f32) / (Y.len() as i32) as f32) as f32;
    ...
    */
    ...
    pub fn split(
        ...
        let mut dataset_left: Matrix = Matrix {
            data: vec_left,
            row: row_left,
            col: n_cols,
        };
        let mut dataset_right: Matrix = Matrix {
            data: vec_right,
            row: row_right,
            col: n_cols,
        };
